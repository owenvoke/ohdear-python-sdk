...
def test_enables_check():
    with requests_mock.Mocker() as m:
        ...
        assert certificate.get('certificate_details').get('issuer') == "Let's Encrypt Authority X3"
        assert certificate.get('certificate_details').get('valid_from') == '2019-09-10 15:16:01'
        assert certificate.get('certificate_details').get('valid_until') == '2019-12-09 15:16:01'
        assert type(certificate.get('certificate_checks')) is list
        assert certificate.get('certificate_checks')[0].get('type') == 'notFound'
        assert certificate.get('certificate_checks')[0].get('label') == 'Certificate present'
        ...
        assert certificate.get('certificate_chain_issuers')[0] == "US, Let's Encrypt, Let's Encrypt Authority X3"
